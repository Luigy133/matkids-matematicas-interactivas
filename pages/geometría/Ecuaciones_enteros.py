...
st.set_page_config(
    page_title="Ecuaciones con Enteros",
    page_icon="🧮",
    layout="centered"
)
...
st.markdown("""
<style>
    /* Fondo principal: degradado morado suave */
    .stApp {
    ...
    /* Títulos en morado oscuro para buen contraste */
    h1, h2, h3 {
...
def generar_ecuacion(rango, nivel):
    ...
    Retorna un diccionario con toda la info del problema.
    """
    if nivel == "Fácil (solo positivos)":
        ...
        if tipo == 'x+a=b':
            ...
            b = a - x
        # Evitamos b negativos en modo fácil
        if b < 0:
            tipo = 'x+a=b'
            b = x + a
...
def dibujar_recta_ecuacion(x_sol, a, b, tipo, mostrar_resultado=False):
    ...
    Muestra cómo el valor de x se ubica y verifica la igualdad.
    """
    ...
    # Flecha que muestra el "despeje" según el tipo
    if mostrar_resultado:
        if tipo == 'x+a=b':
            # Restamos 'a': de b hacia x
            color = '#8e24aa'
...
with st.expander("📚 ¿Cómo resolver ecuaciones con enteros? (Haz clic para aprender)"):
    ...
    Siempre puedes **verificar** tu solución: reemplaza **x** en la ecuación original y comprueba que ambos lados sean iguales.
